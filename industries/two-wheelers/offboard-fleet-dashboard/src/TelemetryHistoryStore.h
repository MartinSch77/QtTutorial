...
// Thin wrapper around a SQLite-backed history table, standing in for a
// bike-share/delivery fleet back-office's telemetry archive. Kept independent
// of any UI so it can be unit tested against an in-memory database.
...
public:
    ...
    bool recordSample(const QString& vehicleId, qint64 timestampMs, double speedKph, double batteryPercent);
