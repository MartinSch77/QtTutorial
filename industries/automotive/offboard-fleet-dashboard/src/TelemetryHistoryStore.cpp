                                          ...
                                          double fuelPercent)
{
    ...
    if (!query.prepare(QStringLiteral(
            "INSERT INTO telemetry_history (vehicle_id, timestamp_ms, speed_kph, fuel_percent) "
            "VALUES (?, ?, ?, ?)"))) {
        return false;
    }
    ...
    query.addBindValue(fuelPercent);
    return query.exec();
}
...
std::vector<double> TelemetryHistoryStore::recentSpeeds(const QString& vehicleId, int limit) const
{
    ...
    QSqlQuery query(m_db);
    if (!query.prepare(QStringLiteral(
            "SELECT speed_kph FROM telemetry_history WHERE vehicle_id = ? "
...
int TelemetryHistoryStore::sampleCount(const QString& vehicleId) const
{
    ...
    QSqlQuery query(m_db);
    if (!query.prepare(QStringLiteral("SELECT COUNT(*) FROM telemetry_history WHERE vehicle_id = ?"))) {
        return 0;
    }
    query.addBindValue(vehicleId);
